...
def extract_orf(fastainputpath, fastaoutputpath, orfminlen, startcodon, geneticcode):
    subprocess.run([constants.orffinderpath, "-in", fastainputpath, "-out", fastaoutputpath, "-ml", str(orfminlen), "-s", str(startcodon), "-g", str(geneticcode)],
    stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT)
    
    
def launch_soap(configoutpath, kmerlen, outputpath_prefix, threads):
    subprocess.run([constants.soappath, "all", "-s", configoutpath, "-o", outputpath_prefix+"_temp", "-K", str(kmerlen), "-p", str(threads)],
    stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT)
