...
if __name__ == "__main__":      
    ...
    for cluster in clusters:
        ...
        if "combine" not in logfile.contents["final"]["progress"]["assembly"][cluster].keys():
            ...
            threadpool)
            #extract Clstr file
            seqtoretain = postprocess.cluster_seq_extractor( np.round(((kmer_max - kmer_min) / (kmer_step*2)) +1 ) ,os.path.join(cluster_assemblydir, "combined", f"c{cluster}_mk_nr_cds.fasta.clstr")) #consensus threshold = number of kmers/2. rounded up 
